...
export function showDashboardPanel(context: vscode.ExtensionContext, result: ScanResult): void {
...
export function updateDashboardPanel(result: ScanResult): void {
  if (currentPanel) {
    currentPanel.webview.html = getDashboardHtml(result);
  }
}

function getDashboardHtml(result: ScanResult): string {
  ...
  for (const v of result.vulnerabilities) {
    if (v.highestSeverity > maxSeverity) { maxSeverity = v.highestSeverity; }
  }
  ...
  if (riskScore >= 70) { riskColor = '#ef4444'; riskLabel = 'Critical'; }
  else if (riskScore >= 40) { riskColor = '#f97316'; riskLabel = 'High'; }
  else if (riskScore >= 20) { riskColor = '#facc15'; riskLabel = 'Medium'; }
  else if (riskScore > 0) { riskColor = '#38bdf8'; riskLabel = 'Low'; }
  ...
  let safePkgsHtml = '';
  const safePackages = result.packages.filter(p =>
    !result.vulnerabilities.find(v => v.package.name === p.name)
  );
...
<body>
  <div class="container">
    ...
    <div style="text-align:center; margin-top:24px; padding-top:16px; border-top:1px solid rgba(255,255,255,0.06);">
      <div style="color:#475569; font-size:9px; text-transform:uppercase; letter-spacing:2px;">Powered by OSV.dev · RippleGuard Security</div>
    </div>
  </div>
</body>
</html>`;
