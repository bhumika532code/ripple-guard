...
// ============================================================================
// WebNode Sentinel — Main Extension Entry Point
// ============================================================================
// Global state
...
function activate(context) {
    console.log('[WebNode Sentinel] Extension activated!');
    ...
    vscode.window.registerTreeDataProvider('webnodeDependencies', treeProvider);
    // --- 3. Register commands ---
    context.subscriptions.push(vscode.commands.registerCommand('webnode.scanNow', async () => {
        await scanWorkspace();
    }));
    context.subscriptions.push(vscode.commands.registerCommand('webnode.fixPackage', () => {
    ...
    for (const glob of manifestGlobs) {
        ...
        watcher.onDidChange((uri) => {
            console.log(`[WebNode] File changed: ${uri.fsPath}`);
            scanFile(uri.fsPath);
        });
        watcher.onDidCreate((uri) => {
            console.log(`[WebNode] File created: ${uri.fsPath}`);
            scanFile(uri.fsPath);
        });
        watcher.onDidDelete((uri) => {
            console.log(`[WebNode] File deleted: ${uri.fsPath}`);
...
async function scanFile(filePath) {
    ...
    try {
        ...
        if (criticalVulns.length > 0) {
            vscode.window.showWarningMessage(`🔴 WebNode: ${criticalVulns.length} CRITICAL vulnerabilit${criticalVulns.length === 1 ? 'y' : 'ies'} detected in ${path.basename(filePath)}!`, 'Show Details').then(choice => {
                if (choice === 'Show Details') {
                    vscode.commands.executeCommand('webnodeDependencies.focus');
                }
            });
        }
    }
    catch (err) {
        console.error(`[WebNode] Scan failed for ${filePath}:`, err);
    }
}
/**
...
async function scanWorkspace() {
    ...
    if (manifests.length === 0) {
        vscode.window.showInformationMessage('WebNode: No manifest files found in this workspace.');
        return;
    }
...
function deactivate() {
    console.log('[WebNode Sentinel] Extension deactivated.');
}
