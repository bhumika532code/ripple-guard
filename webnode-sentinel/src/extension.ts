...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('webnode.scanNow', async () => {
      await scanWorkspace();
    })
  );

  context.subscriptions.push(
    vscode.commands.registerCommand('webnode.fixPackage', () => {
  ...
  try {
    ...
    if (criticalVulns.length > 0) {
      vscode.window.showWarningMessage(
        `🔴 WebNode: ${criticalVulns.length} CRITICAL vulnerabilit${criticalVulns.length === 1 ? 'y' : 'ies'} detected in ${path.basename(filePath)}!`,
        'Show Details'
      ).then(choice => {
